...
function [loc_est,c_est,err,tdoa_mat] = loc_est_hyp(zdata,xpos,ypos,start_sample,end_sample,FS,clist,N,plotting)
...
%     end 
   ...
   %calculate the intersections of all hyperbolas and circles
    ...
    if ~isempty(x_intlist) && ~isempty(y_intlist)
        ...
        figure
        x_hist = histfit(real(x_intlist((x_intlist<5000 & x_intlist>-5000))),round(length(x_intlist((x_intlist<5000 & x_intlist>-5000)))/2),'kernel'); %fit a kernel distribution to histogram of x-intersection values
        ...
        figure
        y_hist = histfit(real(y_intlist((x_intlist<5000 & x_intlist>-5000))),round(length(y_intlist((x_intlist<5000 & x_intlist>-5000)))/2),'kernel'); %fit a kernel distribution to histogram of y-intersection values
