...
%     end 
   ...
   %calculate the intersections of all hyperbolas and circles
    ...
    if ~isempty(x_intlist) && ~isempty(y_intlist)
        ...
        figure
        x_hist = histfit(real(x_intlist((x_intlist<5000 & x_intlist>-5000))),round(length(x_intlist((x_intlist<5000 & x_intlist>-5000)))/1),'kernel'); %fit a kernel distribution to histogram of x-intersection values
        ...
        figure
        y_hist = histfit(real(y_intlist((x_intlist<5000 & x_intlist>-5000))),round(length(y_intlist((x_intlist<5000 & x_intlist>-5000)))/1),'kernel'); %fit a kernel distribution to histogram of y-intersection values
    ...
    if isempty(xloclist) %if no peaks in kernel function is found, use mode method for x,y estimates
            loc_est = [mode(round(x_intlist((x_intlist<5000 & x_intlist>-5000)),3)) loc_est];
    end
        
    if isempty(yloclist)
            loc_est = [loc_est mode(round(y_intlist((x_intlist<5000 & x_intlist>-5000)),1))];
    end
    
    %------------------------------------------------------------------------------------------------------------------------------------------------%
    %calculates the error in tdoa matrices between estimation and a simulated event at the estimated location
    %selects the propagation speed/estimation location with the lowest error
    
    tdoa_sim_mat = tdoa_sim(loc_est(1),loc_est(2),c0,xpos,ypos); %calculated the tdoa matrix as if the event occurred at the estimated location with the assumed propagation speed. 
    error(ccount) = sqrt(sum(sum((tdoa_mat - tdoa_sim_mat).^2))); %calculate the error of the simulated tdoa matrix and the actual tdoa matrix. 
    c_estlist(ccount) = c0;
