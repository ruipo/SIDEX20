...
%node_trim = 110000;
%toffset = 1.6956e-04; % define time offset between wired and stand-alone geophone data (derived imperically.
%t1 = 32000;%1/28_1
%t2 = 33500;%1/28_1
%t1 = 8500;%1/28_2
%t2 = 10000;%1/28_2

%Table = readtable('/Volumes/RUIC_Backup/SIDEX20_data/SIDEx_node_data/transients_with_4_nodes_TD/2020-02-01_23_55_31.csv', 'HeaderLines',1,'Format','%D%f%f%f%f%f%f%f%f%f%f%f%f');
%t_trim = 0;
...
if node_trim > 0
    tn = tn(1:size(zdata,1));
    z_node = z_node(1:size(zdata,1),:);
    x_node = x_node(1:size(xdata,1),:);
    y_node = y_node(1:size(ydata,1),:);
end
...
%try
    [loc_est,c_est,err,tdoa_mat] = loc_est_calib_testfn(zdatafilt,xdatafilt,ydatafilt,zdata,xdata,ydata,xpos,ypos,1,length(zdatafilt),FS_node,c_range,N,plotting,calib_act,noise_mat,plotmap,1);
%catch
%    disp('Skipping this...')
%end
