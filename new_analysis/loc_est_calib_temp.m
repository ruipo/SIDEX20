...
for ccount = 1:length(clist) %loop throught list of propagation speeds
    ...
    [tdoa_mat,~,~] = tdoa_sidex_temp(zdata,xdata,ydata,start_sample,end_sample,FS,'hilbert'); %calculate tdoa matrix based on the propagation speed. 
...
%     end 
   ...
   %calculate the intersections of all hyperbolas and circles
    x_intlist = [];
    y_intlist = [];
    if ((~isempty(x_mat)) && (~isempty(y_mat)))
