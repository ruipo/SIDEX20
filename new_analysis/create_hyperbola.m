...
theta1_r        = linspace(-pi*0.499,pi*0.499,N/2);
theta2_r        = pi + theta1_r;
switch hyp_type,
case 1,
    slope = b/a;
    hyperbola_x1_r  = 0 + a./cos( theta1_r );
    hyperbola_x2_r  = 0 + a./cos( theta2_r );
    hyperbola_y1_r  = 0 + b*tan( theta1_r );
    hyperbola_y2_r  = 0 + b*tan( theta2_r );
case 2,
    slope = a/b;
    hyperbola_x1_r  = 0 + b*tan( theta1_r );
    hyperbola_x2_r  = 0 + b*tan( theta2_r );
    hyperbola_y1_r  = 0 + a./cos( theta1_r );
    hyperbola_y2_r  = 0 + a./cos( theta2_r );
otherwise,
    error( 'iligal value for variable "hyp_type", choose 1 for a horizontal hyperbolam or 2 for a vertical one' );
end
points(1:2,:)   = R * [hyperbola_x1_r;hyperbola_y1_r] + [X0.*ones(1,length(hyperbola_x1_r));Y0.*ones(1,length(hyperbola_x1_r))];
points(3:4,:)   = R * [hyperbola_x2_r;hyperbola_y2_r] + [X0.*ones(1,length(hyperbola_x2_r));Y0.*ones(1,length(hyperbola_x2_r))];
