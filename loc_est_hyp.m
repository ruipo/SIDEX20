...
function [loc_est,c_est,err,tdoa_mat] = loc_est_hyp(zdata,xdata,ydata,xpos,ypos,start_sample,end_sample,FS,clist,N,plotting)
...
for ccount = 1:length(clist) %loop throught list of propagation speeds
    ...
    [tdoa_mat,~,~] = tdoa_sidex(zdata,xdata,ydata,start_sample,end_sample,FS,'max'); %calculate tdoa matrix based on the propagation speed. 
